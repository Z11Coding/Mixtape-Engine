...
class APEntryState extends FlxState
{
		...
		var yamlImport = new PsychUIButton(0, 0, "Import YAML", function()
		{
			...
			fileDialog.load('', [new FileFilter('YAML', 'yaml')], function()
			{
				try
				{
					...
					gameSettings.progression_balancing = loadedFile.get('progression_balancing');
					gameSettings.accessibility = loadedFile.get('accessibility');
					gameSettings.mods_enabled = stringToBool(loadedFile.get('mods_enabled'));
					gameSettings.deathlink = stringToBool(loadedFile.get('deathlink'));
					gameSettings.starting_song = loadedFile.get('starting_song');
					gameSettings.unlock_type = loadedFile.get('unlock_type');
					gameSettings.unlock_method = loadedFile.get('unlock_method');
					gameSettings.ticket_percentage = Std.parseInt(loadedFile.get('ticket_percentage'));
					gameSettings.ticket_win_percentage = Std.parseInt(loadedFile.get('ticket_win_percentage'));
					gameSettings.chart_modifier_change_chance = Std.parseInt(loadedFile.get('chart_modifier_change_chance'));
					gameSettings.trapAmount = Std.parseInt(loadedFile.get('trapAmount'));
					gameSettings.bbcWeight = Std.parseInt(loadedFile.get('bbcWeight'));
					gameSettings.ghostChatWeight = Std.parseInt(loadedFile.get('ghostChatWeight'));
					gameSettings.svcWeight = Std.parseInt(loadedFile.get('svcWeight'));
					gameSettings.tutorialWeight = Std.parseInt(loadedFile.get('tutorialWeight'));
					gameSettings.fakeTransWeight = Std.parseInt(loadedFile.get('fakeTransWeight'));
					gameSettings.shieldWeight = Std.parseInt(loadedFile.get('shieldWeight'));
					gameSettings.MHPWeight = Std.parseInt(loadedFile.get('MHPWeight'));
				}
				catch(e:Exception)
				{
					trace(e.stack);
	...
	override function update(elapsed:Float)
	{
		...
		bpmTxt.text = 
			...
			+ "\nMods Enabled: "
			+ gameSettings.mods_enabled
			+ "\nDeathlink: "
			+ gameSettings.deathlink
			+ "\nStarting Song: "
			+ gameSettings.starting_song
			+ "\nUnlock Type: "
			+ gameSettings.unlock_type
			+ "\nUnlock Method: "
			+ gameSettings.unlock_method
			+ "\nTicket Percentage: "
			+ gameSettings.ticket_percentage
			+ "\nTicket Win Percentage: "
			+ gameSettings.ticket_win_percentage
			+ "\nChart Modifier Change Chance: "
			+ gameSettings.chart_modifier_change_chance
			+ "\nTrap Amount: "
			+ gameSettings.trapAmount
			+ "\nBlue Balls Curse Weight: "
			+ gameSettings.bbcWeight
			+ "\nGhost Chat Weight: "
			+ gameSettings.ghostChatWeight
			+ "\nStreamer Vs. Chat Weight: "
			+ gameSettings.svcWeight
			+ "\nTutorial Weight: "
			+ gameSettings.tutorialWeight
			+ "\nFake Transition Weight: "
			+ gameSettings.fakeTransWeight
			+ "\nShield Weight: "
			+ gameSettings.shieldWeight
			+ "\nMax HP Weight: "
			+ gameSettings.MHPWeight;
	}
}
