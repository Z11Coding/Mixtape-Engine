                ...
                default:
                    ...
                    if (APEntryState.gameSettings.FNF.mods_enabled)
                    {
                        for (i in 0...WeekData.weeksList.length) {
                            ...
                            {
                                toList.remove(song[0]); // To remove dups
                                toList.push(song[0] + (StringTools.trim(leWeek.folder) != "" ? " (" + leWeek.folder + ")" : "")); // To add the folder name if it's not empty
                                toList.remove(APEntryState.gameSettings.FNF.starting_song); // To remove Tutorial because it keeps re-adding itself
                            }
                        }
                    }
            }
